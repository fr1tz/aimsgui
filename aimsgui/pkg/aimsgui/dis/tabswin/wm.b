...
init(ctxt: ref Draw->Context, argv: list of string)
{
	...
	win := wmclient->window(ctxt, "mainwin", Wmclient->Plain);
	dr := win.displayr;
	wmclient->win.reshape(((dr.min.x+200,dr.min.y+18), (dr.max.x,dr.max.y)));
	...
	argv = tl argv;
	if(argv == nil)
		argv = "mipsgui/mainwin/toolbar" :: nil;
...
setfocus(win: ref Wmclient->Window, new: ref Client)
{
	...
	if(new != nil){
		controlevent("kbdfocuschange " + string new.id);
		new.ctl <-= "raise";
		new.ctl <-= "haskbdfocus 1";
		kbdfocus = new;
	} else {
		controlevent("kbdfocuschange none");
		kbdfocus = nil;
	}
}
