...
title_cfg := array[] of {
	"frame .Wm_t -bg #FFFFFF -borderwidth 1",
	"label .Wm_t.title -anchor w -bg #FFFFFF -fg #000000",
	"button .Wm_t.e -bitmap @/icons/tinytk/exit.bit -command {send wm_title exit} -takefocus 0",
	"pack .Wm_t.e -side right",
	"bind .Wm_t <Button-1> {send wm_title move %X %Y}",
	"bind .Wm_t <Double-Button-1> {send wm_title lower .}",
	"bind .Wm_t <Double-Button-2> {send wm_title exit}",
	"bind .Wm_t <Button-3> {send wm_title size}",
	...
	"bind .Wm_t <Double-Button-2> {send wm_title exit}",
	"bind .Wm_t.title <Button-3> {send wm_title size}",
	"bind .Wm_t.title <Motion-Button-1> {}",
	"bind .Wm_t.title <Motion> {}",
	#"bind . <FocusIn> {.Wm_t configure -bg #000000;"+
	#	".Wm_t.title configure -bg #000000 -fg #FFFFFF; update}",
	#"bind . <FocusOut> {.Wm_t configure -bg #FFFFFF;"+
	#	".Wm_t.title configure -bg #FFFFFF -fg #000000; update}",
};
...
new(top: ref Tk->Toplevel, buts: int): chan of string
{
	...
	# Make sure the default titlebar widgets exist, just don't show them 

	for(i := 0; i < len title_cfg; i++)
		cmd(top, title_cfg[i]);

	if(buts & OK)
		cmd(top, "button .Wm_t.ok -bitmap @/icons/tinytk/ok.bit"+
			" -command {send wm_title ok} -takefocus 0; pack .Wm_t.ok -side right");

	if(buts & Resize)
		cmd(top, "button .Wm_t.m -bitmap @/icons/tinytk/maxf.bit"+
			" -command {send wm_title size} -takefocus 0; pack .Wm_t.m -side right");

	if(buts & Help)
		cmd(top, "button .Wm_t.h -bitmap @/icons/tinytk/help.bit"+
			" -command {send wm_title help} -takefocus 0; pack .Wm_t.h -side right");

	if(buts & Hide)
		cmd(top, "button .Wm_t.top -bitmap @/icons/tinytk/task.bit"+
