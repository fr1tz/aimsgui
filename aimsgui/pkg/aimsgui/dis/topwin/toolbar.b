		...
		exec, task: chan of string): ref Tk->Toplevel
{
	tkopts: string;
	if(1)
		tkopts = "-background #DADADA";
	(tbtop, nil) = tkclient->toplevel(ctxt, tkopts, nil, Tkclient->Plain);
	...
	if (startmenu) {
		cmd(tbtop, "image create bitmap iconimage" +
			" -file @/icons/aimsgui/logo.small.bit" +
			" -maskfile @/icons/aimsgui/logo.small.mask.bit"); 
		cmd(tbtop, "menubutton .toolbar.start -menu .m -image iconimage");
		cmd(tbtop, "pack .toolbar.start -side left -fill y");
	}
...
{
	ctxt := shctxt.copy(0);
	ctxt.run(shell->stringlist2list("run"::"/dis/aimsgui/topwin/wmsetup"::nil), 0);
