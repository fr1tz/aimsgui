...
init(ctxt: ref Draw->Context, argv: list of string)
{
	...
	if(ctxt.wm == nil)
		buts = Wmclient->Plain;
	win := wmclient->window(ctxt, "AIMSGUI", buts);
	dr := win.displayr;
	wmclient->win.reshape(((dr.min.x, dr.min.y), (dr.max.x, dr.max.y)));
	...
	argv = tl argv;
	if(argv == nil)
		argv = "aimsgui/topwin/toolbar" :: nil;
