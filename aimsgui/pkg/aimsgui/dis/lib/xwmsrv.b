...
zorder: ref Client; # packed clients, linked by znext.
bottom: ref Client; # client at the end of zorder.
			...
			req: chan of (ref Client, array of byte, Sys->Rwrite))
{
	...
	for(;;)alt{
	(cmd, rc) := <-wmreq =>
		#sys->fprint(sys->fildes(2), "wmsrv: wm(): <-wmreq: %s\n", cmd);
		token := int cmd;
	...
	(nil, data, fid, wc) := <-ctlio.write =>
		#sys->fprint(sys->fildes(2), "wmsrv: wm(): <-ctlio.write: %s\n", string data);
		c := findfid(clients, fid);
		if(wc != nil){
			if(c == nil){
				alt{
