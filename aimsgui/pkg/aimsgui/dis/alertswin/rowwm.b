...
init(ctxt: ref Draw->Context, argv: list of string)
{
	...
	wmsrv = load Xwmsrv Xwmsrv->PATH;
	if(wmsrv == nil)
		badmodule(Xwmsrv->PATH);
	...
	if(ctxt.wm == nil)
		buts = Wmclient->Plain;
	win := wmclient->window(ctxt, "Alerts", buts);
	dr := win.displayr;
	wmclient->win.reshape(((dr.min.x,dr.min.y+18), (dr.min.x+196,dr.max.y)));
	wmclient->win.onscreen("exact");
	if(win.image == nil){
	...
	p := <-wmctxt.ptr =>
		...
		if(c != nil) {
			if(p.buttons == 0)
			{
				#sys->fprint(sys->fildes(2), "c: %d\n", c.id);
				if(ptrfocus != nil && c != ptrfocus)
					ptrfocus.ptr <-= p;
				ptrfocus = c;
			}
			else
	...
	(c, data, rc) := <-req =>
		...
		if(rc == nil){
			...
			pack_windows(screen.image.r, screen.image.r);
			for(z := wmsrv->top(); z != nil; z = z.znext)
...
handlerequest(win: ref Wmclient->Window, wmctxt: ref Wmcontext, c: ref Client, req: string): string
{
	...
	"cursor" =>
		...
		if(n != 6 && n != 1)
			return "bad arg count";
		#sys->fprint(sys->fildes(2), "wm: cursor: %s\n", req);
		c.cursor = req;
...
pack_windows(oldr, newr: Rect)
{
	z: ref Client;
	for(z = wmsrv->top(); z != nil; z = z.znext){
		if(z.attrs != nil && z.attrs[0] == "header"){
			z.top();
			break;
		}
	}
	for(z = wmsrv->top(); z != nil; z = z.znext){
		if(z.attrs != nil && z.attrs[0] == "footer"){
			z.bottom();
			break;
		}
	}
...
Fix: con 1000;
reshape_floating(oldr, newr: Rect)
{
	mx := Fix;
	if(oldr.dx() > 0)
...
reshaped(win: ref Wmclient->Window)
{
	...
	newr := win.image.r;
	screen = makescreen(win.image);
	reshape_floating(oldr, newr);
	pack_windows(oldr, newr);
}
...
reshape(c: ref Client, tag: string, r: Rect): string
{
	...
	# don't change client place in the z-order list if it's already in it...
	listed := 0;
	for(z := wmsrv->top(); z != nil; z = z.znext) {
		if(z == c) {
			listed = 1;
	...
	if(!listed)
		c.bottom();
	# if the size changed (and it's not a floating win): we need to repack all windows...
	if(!samesize && tag == ".")
		pack_windows(screen.image.r, screen.image.r);
	return nil;
}
...
{
	bgcol := int 16rDADADAFF;
	f := sys->open("/lib/look/bgcolor", sys->OREAD);
	if(f != nil)
	{
		col := array[8] of byte;
		if(sys->read(f, col, len col) == len col)
			(bgcol, nil) = str->toint(string col, 16);
	}
	screen = Screen.allocate(img, img.display.color(bgcol), 0);
