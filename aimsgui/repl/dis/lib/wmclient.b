...
WMCLIENTBORDERUP: con int 16r888888FF;
WMCLIENTBORDERDOWN: con int 16r888888FF;
...
putimage(w: ref Window, i: ref Image)
{
	if(w.screen != nil && i == w.screen.image)
		return;
	w.screen = Screen.allocate(i, w.display.color(Draw->Red), 0);
	ir := i.r.inset(w.bd);
	if(ir.dx() < 0)
		ir.max.x = ir.min.x;
	if(ir.dy() < 0)
...
{
	#sys->fprint(sys->fildes(2), "wmclient: Window.wmctl(): %s\n", req);
	(c, next) := qword(req, 0);
	case c {
	"exit" =>
...
{
	#sys->fprint(sys->fildes(2), "wmclient: wmreq(): %s\n", req);
	name: string;
	if(req != nil && req[0] == '!'){
		(name, e) = qword(req, e);
		if(name != ".")
	...
	if(w.ctxt.connfd != nil){
		#sys->fprint(sys->fildes(2), "wmclient: wmreq(): have wm\n");
	...
	#sys->fprint(sys->fildes(2), "wmclient: wmreq: have no wm\n");
