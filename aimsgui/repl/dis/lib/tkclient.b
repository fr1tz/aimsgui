...
getelementcolor(elementname: string): string
{
	f := sys->open("/lib/look/"+elementname, sys->OREAD);
	if(f != nil)
	{
		col := array[8] of byte;
...
toplevel(ctxt: ref Draw->Context, topconfig: string, title: string, buts: int): (ref Tk->Toplevel, chan of string)
{
	...
	opts := "";
	col := getelementcolor("bgcolor");
	if(col != nil)
		opts += " -background #" + col;
	col = getelementcolor("fgcolor");
	if(col != nil)
		opts += " -foreground #" + col;
	col = getelementcolor("activebgcolor");
	if(col != nil)
	{
		opts += " -activebackground #" + col;
		opts += " -selectcolor #" + col;
	}
	col = getelementcolor("activefgcolor");
	if(col != nil)
		opts += " -activeforeground #" + col;
	if((buts & Plain) == 0)
		opts += " -borderwidth 1 -relief raised ";
	#sys->print("tkclient->toplevel(): conf: %s\n", opts);
